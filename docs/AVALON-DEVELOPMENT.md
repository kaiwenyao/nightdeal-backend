...
| 阵营 | 颜色 | 核心目标 | 胜利条件（摘要） |
| --- | --- | --- | --- |
| 好人 | 蓝方 (GOOD) | 成功执行任务并保护梅林 | **须同时满足**：① 累计 **3** 次任务成功；② 终局「刺杀」环节中，梅林**未被**刺客准确指认并刺杀。 |
| 坏人 | 红方 (EVIL) | 破坏任务或刺杀梅林 | **满足以下任一**即立即获胜：① **任务失败**：累计 **3** 次任务失败；② **刺杀成功**：蓝方已达成 3 次任务成功后，刺客准确找出并刺杀梅林；③ **延误发车（憋死）**：同一任务轮次内，组队提案被连续否决 **5** 次（含第 5 次；反对多数或**平票均视为否决**，见 §2.6）。 |

#### 2.1.2 详细规则拆解（产品 / 新手说明 / 开发备注）

**蓝方（好人）**

- 蓝方胜利是**被动且滞后**的：即使率先达成 **3** 次任务成功，也**不得**立即宣告蓝方获胜，必须**强制进入刺杀环节**。
- **只有**在刺杀环节中刺客**未能**准确刺杀梅林时，蓝方才算赢下本局。

**红方（坏人）**

- **任务失败 3 次**：当第 **3** 次任务失败（失败票达到该轮 `requiredFails`，见 §2.4）时，对局**立刻结束**，红方获胜；**无需**进入刺杀。
- **刺杀成功**：蓝方已达成 3 次任务成功后，若刺客指认的目标**确为梅林**，红方立即获胜。
- **连续 5 次否决**：在同一**任务轮次**（`round`）的组队阶段，若提案连续 **5** 次被公投否决，红方立即获胜；用于避免好人无限期否决组队。实现上与阶段机 `attemptNo`、§2.5 / §2.6 边界裁定一致。

### 2.2 角色定义与视野

| 角色 | 阵营 | 视野说明 |
| --- | --- | --- |
| 梅林 (Merlin) | GOOD | 看见所有红方（**莫德雷德除外**）；**可以看见奥伯伦**。误写「奥伯伦除外」为错 |
...
| 奥伯伦 (Oberon) | EVIL | **不看同伴，同伴也不看 ta**；梅林可识别其为坏人（与莫德雷德对梅林隐身不同） |
...
### 2.3 人数与角色配置表（派西维尔常驻版）

以下为本项目文档采用的 **派西维尔局局常驻** 口径（配牌说明与 UI 文案对齐）；总人数、蓝/红方人数与默认必含如下，可选栏为常见变体。**默认牌堆须与** `getDefaultConfig(playerCount)`（`role-config.schema.ts`）**人数守恒一致**。

| 总人数 | 蓝方人数 | 红方人数 | 默认配置（必含） | 可选额外（替换对应阵营普通角色） |
| --- | --- | --- | --- | --- |
| 5 | 3 | 2 | 梅林 + 派西维尔 + 1 忠臣 + 刺客 + 莫甘娜 | — |
...
| 场景 | 裁定 |
| --- | --- |
| 公投平票 (赞成 == 反对) | **视为否决**。若当前 `attemptNo < 5`：进入下一轮提案（`attemptNo +1`，队长顺延）。若当前 **`attemptNo == 5`**：**不再 +1**，直接 `GAME_OVER`（`EVIL` / `FIVE_REJECTIONS`），见下行。 |
